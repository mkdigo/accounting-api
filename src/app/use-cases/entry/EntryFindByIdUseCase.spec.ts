...
import { EntryFindByIdUseCase } from './EntryFindByIdUseCase';
import { Entry } from '@/domain/entities/Entry';
import { EntryCreateUseCase } from './EntryCreateUseCase';

describe('EntryFindByIdUseCase', () => {
  it('should be able to find a entry by id', async () => {
    ...
    const foundEntry = await entryFindByIdUseCase.execute(entry.id);
    expect(foundEntry).toBeInstanceOf(Entry);
    expect(foundEntry?.id).toBe(entry.id);
    expect(foundEntry?.debit_name).toBe('Rent');
    expect(foundEntry?.credit_name).toBe('Bank');
    expect(foundEntry?.value.toNumber()).toBe(2000.0);
  });
});
